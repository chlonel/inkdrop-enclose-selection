...
export const encloseSelections = (
  ...
  const replacedSelections = selectionStrings.map(
    (selectionString) => leftEnclosure + selectionString + rightEnclosure
  );
  cm.replaceSelections(replacedSelections);

  const addedEnclosureCounter = {} as Record<number, number>;
  const insideEnclosureSelections = beforeReplacementSelections.map(
    (selection) => {
      const sortedPositionTypes = positionTypes.sort((a, b) => {
        return (
          selection[a].line - selection[b].line ||
          selection[a].ch - selection[b].ch
        );
      });
